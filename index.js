...
function fenomenSil(fenomenler,index) {
  const Parametre2 = fenomenler;
  Parametre2.splice(index,1);
  return Parametre2;
...
function fenomenEkle(dizi,sırano,isim,takipci,gönderi,platform) {
  const Parametre3 = dizi;
  let nesne = {
    "number": sırano,
    "profile": isim,
    "followers": takipci,
...
function enFenomenler(fenomenler) {
  ...
  for(let i = 0; i<fenomenler.length;i++){
    if (fenomenler[i].followers>100000000) {
      Parametre4.push(fenomenler[i].profile);
    }
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platformadi){
    ...
    let profil;
    for(let i=0;i<fenomenler.length;i++){
    if(fenomenler[i].platform == platformadi && fenomenler[i].posts>max){
      max = fenomenler[i].posts;
      profil = fenomenler[i].profile;
